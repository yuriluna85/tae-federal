...
*   🛡️ **Marcações Schema.org JSON-LD**: Injeção de dados estruturados Schema.org (`Article`, `BreadcrumbList`, `FAQPage`, `SoftwareApplication` e `WebSite`) no `index.html` e em todos os 15 artigos da seção editorial para otimizar o entendimento do Googlebot.
*   🌐 **Navegação & Sitemap XML**: Atualizado o `sitemap.xml` para incluir todas as 15 novas URLs de artigos e adicionada a aba "📚 Artigos & Guias" na navegação principal do portal.
*   💰 **Auditoria AdSense Readiness (Skill `adsense-readiness`)**: Executada a auditoria automatizada via skill `adsense-readiness`, com elevação da Nota de Elegibilidade do AdSense de **55.0/100 para 100.0/100 (Status Verde: Pronto para Submissão)**, erradicando a rejeição por "Conteúdo de Baixo Valor".

### [02/07/2026] - Ajustes de Tabelas e Equivalências CCE/CD
