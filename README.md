...
*   🎯 **Pontuação RSC & Nível Pleiteado**: Indicadores de pontos individuais exibidos dinamicamente ao lado de cada caixa de quantidade de atividades no barema de RSC, otimizando a visualização e usabilidade. Adicionado dropdown de seleção para o nível de RSC pleiteado (RSC-I a RSC-VII) na aba de pontuação, personalizando a mensagem de status da meta e integrando-o ao arquivo de salvamento/carregamento da sessão em formato JSON. Incorporadas as travas normativas: a) **Exigência de Dispersão**, impedindo homologação caso todos os pontos estejam concentrados em um único eixo (mínimo de 2 eixos ativos); e b) **Eixo Obrigatório do Doutorado**, exigindo pontuação mínima no Eixo VI (Produção e Difusão Técnica) para pleitear o RSC-VII (Doutorado).
