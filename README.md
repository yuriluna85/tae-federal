...
*   🌐 **Navegação & Rastreabilidade de Crawlers**: Inserida a ligação direta para a Central de Guias e Artigos (`artigos/index.html`) e `tutorial.html` no rodapé institucional (`.footer-links`) do `index.html`, garantindo que os crawlers do Google AdSense alcancem e indexem 100% dos 16 artigos explicativos da carreira.
*   💰 **Adequação Anti-"Conteúdo de Baixo Valor"**: Homologadas as seções textuais explicativas abaixo da calculadora e integração com o hub de artigos para erradicação completa do diagnóstico de conteúdo superficial.
