...
*   ✈️ **Simulador de Diárias**: Novo painel completo para cálculo de diárias nacionais (Decreto nº 11.872/2023) parametrizado por destino (Demais Cidades, Capitais, Brasília/Manaus/RJ/SP) e cargo/função, deduzindo proporcionalmente as parcelas diárias de auxílio-alimentação e auxílio-transporte do servidor em trânsito. Ajuste na regra de descontos para desconsiderar fins de semana (sábados e domingos) e feriados nacionais nos descontos de alimentação e transporte, e integração do calendário nativo auto-exibível sob o clique do usuário.
*   🎯 **Pontuação RSC & Nível Pleiteado**: Indicadores de pontos individuais exibidos dinamicamente ao lado de cada caixa de quantidade de atividades no barema de RSC, otimizando a visualização e usabilidade. Adicionado dropdown de seleção para o nível de RSC pleiteado (**RSC-PCCTAE-I a RSC-PCCTAE-VI**) na aba de pontuação, personalizando a mensagem de status da meta e integrando-o ao arquivo de salvamento/carregamento da sessão em formato JSON. Incorporadas as travas normativas: a) **Exigência de Dispersão**, impedindo homologação caso todos os pontos estejam concentrados em um único eixo (mínimo de 2 eixos ativos); e b) **Eixo Obrigatório do Doutorado**, exigindo pontuação mínima no Eixo VI (Produção e Difusão Técnica) para pleitear o **RSC-PCCTAE-VI (Doutorado)**. Desenvolvimento de **Painel Informativo Dinâmico da Lei do RSC-PCCTAE** (Lei nº 15.367/2026) que exibe de forma contextualizada para cada nível selecionado os requisitos formais acadêmicos, os percentuais de IQ pretendidos e o detalhamento das regras fundamentadas nos **Arts. 12-B, 12-C e 12-D** da legislação.
...
*   💵 **Ajuste de Container do AdSense**: Renomeação das classes CSS de `.adsense-container` para `.patrocinio-container` a fim de evitar bloqueios preventivos de ad-blockers que ocultavam inclusive os placeholders locais informativos ("Espaço Publicitário"). Adicionada a criação do arquivo `ads.txt` na raiz com as diretivas de autorização do AdSense para o ID do publicador do usuário.
